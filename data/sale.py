...
dataField = {
    ...
    "amenities_extra_facet": ["swimming", "power_backup", "lifts", "security", "parking space", "gym and spa", "library"],
    "amenities_facet": ["furnished", "On-site maintanance", "Balcony", "Modular Kitchen", "pvc windows"],
    "date_of_completion": "Dec 2022",
    "sqrft": 1800,
    "rate_per_sqrft": 4600,
    "image_url": ""
}
...
def main():
    ...
    for _ in range(100000):
        document = {
            ...
            "construction_status": bool(random.getrandbits(1)),
        ...
        if (document["city_facet"] in ["Chennai", "New Delhi", "Mumbai", "Bangalore", "Hyderbad"]):
            document["rate_per_sqrft"] = random.randint(120, 400) * 100
        else:
            document["rate_per_sqrft"] = random.randint(2, 100) * 100
        ...
        document["price"] = document["sqrft"] * document["rate_per_sqrft"]
        if(document["construction_status"]):
            document["date_of_completion"] = f"{random.randint(1, 12)}, {random.randint(22, 23)}"
        id += 1
